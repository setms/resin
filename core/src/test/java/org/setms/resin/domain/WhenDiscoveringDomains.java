...
class WhenDiscoveringDomains {
    ...
    void shouldAssignDomainDependenciesViaEvent() {
        ...
        assertThat(domain1, Matchers.not(domain2));
        assertThat(domains.edges().count(), is(1L));
        var domainDependency = domains.edges().findFirst().orElseThrow();
        assertThat(domainDependency.from(), Matchers.is(domain1));
        assertThat(domainDependency.to(), Matchers.is(domain2));
    }
    ...
    void shouldDiscoverGdprMiddlewareDomains() {
        ...
        assertThat(domains.vertices().toList(), Matchers.containsInAnyOrder(servicesDomain, notificationsDomain, formDomain));
